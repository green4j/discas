 ...
 * the key space. The only mutable state is the lease expiry, advanced by {@link #renew}; it is
 * {@code volatile} because a lock is routinely renewed from a keep-alive thread while the work it
 * guards runs on another.
 *
 ...
 * shorten a lease under its owner, which the wall clock alone could do in either direction. That
 * is why only the monotonic reading is tracked here: a holder has no use for the wall-clock
 * deadline it wrote, and offering it would only invite the wrong subtraction.
 * <p>
...
public final class DistributedLock implements Lock {
    ...
    /** Extends the lease via CAS, moving the monotonic expiry {@link #remainingLease()} reports. */
    @Override
    public CompletableFuture<LockWriteResult> renew(final Duration newLeaseTtl) {
        // Anchored before the call, so the reported remainder is a slight underestimate of what
        // renewLock stores a few milliseconds later -- the safe side for a caller deciding when to
        // renew again.
