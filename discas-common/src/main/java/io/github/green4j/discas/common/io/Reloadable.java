...
/**
 * A hot-reloadable source of an immutable value {@code T}: read the current value once
 ...
 * source fires the listener exactly once, a watched one fires on each reload. So a consumer never
 * needs to ask which kind it has, and every kind of watched material shares
 * {@link WatchedFileSource}.
 */
public interface Reloadable<T> {
     ...
     * atomically with reloads) and the new value on every later change. Implementations
     * may invoke the listener on their own reload thread -- a consumer that touches
     * single-threaded state must marshal onto its own thread first.
     */
